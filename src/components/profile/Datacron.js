...
function Datacron ({datacron, size='md', datacrons, affixTextMap, onClick=()=>{}, simple=true, modal=false, test=undefined, result=undefined}){
    ...
    const getBonusCells = () => {
        let image = (url) => {
            ...
            <div className="datacron-primary-icon">
                <div className="datacron-primary-icon__selected-ring"></div>
                <img className="datacron-primary-icon__img datacron-primary-icon__img--is-active" src={url} alt="" loading="lazy"/>
            </div>
        </div>
        }

        return datacron.affix
            .filter(affix => affix.abilityId !== '')
            .map((affix, index) => {
                ...
                return <Item key={index}>
                    <Item.Content>
                    <Item.Image size='tiny' content={image(url)}/>
                        <Item.Header>{categoryName}</Item.Header>
                        <Item.Description>
                            {text}
                        </Item.Description>
                    </Item.Content>
